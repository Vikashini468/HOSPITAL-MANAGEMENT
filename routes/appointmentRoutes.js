...
async function runConsultationPrediction(pool, appointmentId) {
    ...
    if (!flaskRes.ok) {
        ...
        throw new Error("Model service error: " + (err.error || flaskRes.status));
    }
    ...
    /* Store the prediction (table may be created by patientRoutes too) */
    await pool.query(`
    ...
    `, [dataset.patient_id, result.prediction, result.probability, confidence, doctorId, JSON.stringify(payload)]);
